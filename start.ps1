# Spolujízda - PowerShell startup script s debuggingem
# Spouští backend a frontend servery s detailním logováním

# Nastavení encodingu na UTF-8
chcp 65001 | Out-Null
...
if (!(Test-Path "venv")) {
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Host "CHYBA: Nepodarilo se vytvorit virtualni prostredi" -ForegroundColor Red
        Set-Location ..
        Read-Host "Stisknete Enter pro ukonceni"
        exit 1
    }
...
try {
    ...
    Write-Host "CHYBA: Nepodarilo se aktivovat virtualni prostredi" -ForegroundColor Red
    Write-Host "Zkuste povolit spousteni skriptu prikazem:" -ForegroundColor Yellow
...
# Aktualizuj pip
Write-Host "Aktualizuji pip..." -ForegroundColor Yellow
...
if (!(Test-Path "spolujizda.db")) {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        $dbOutput | ForEach-Object { Write-Host $_ -ForegroundColor Red }
        Set-Location ..
        Read-Host "Stisknete Enter pro ukonceni"
        exit 1
    }
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "Backend se pravdepodobne nespusti spravne." -ForegroundColor Yellow
    Set-Location ..
    Read-Host "Stisknete Enter pro ukonceni"
    exit 1
}
...
if ($LASTEXITCODE -eq 0) {
    ...
    $buildTest | ForEach-Object { Write-Host $_ -ForegroundColor Yellow }
}
...
Start-Process powershell -ArgumentList @(
    "-NoExit",
    "-Command",
    "chcp 65001 | Out-Null; Set-Location '$backendPath'; & '.\venv\Scripts\Activate.ps1'; Write-Host 'Backend startuje...' -ForegroundColor Green; python app.py"
)
...
Start-Process powershell -ArgumentList @(
    "-NoExit",
    "-Command",
    "chcp 65001 | Out-Null; Set-Location '$frontendPath'; `$env:BROWSER='none'; Write-Host 'Frontend startuje...' -ForegroundColor Green; npm start"
)
...
Write-Host
Write-Host "Otevrou se dve nova okna s logy serveru" -ForegroundColor Yellow
...
Write-Host "   - Sledujte chybove hlasky v oknech serveru" -ForegroundColor Gray
...
Write-Host "Oteviram prohlizec..." -ForegroundColor Cyan
